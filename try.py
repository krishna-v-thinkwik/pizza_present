...
def check_order():
    ...
    def normalize_topping_name(topping):
        topping = topping.lower().strip()
        mapping = {
            "mushroom": "grilled mushroom",
            "mushrooms": "grilled mushroom",
            "olive": "black olive",
            "olives": "black olive",
            "capsicum": "crisp capsicum",
            "tomato": "fresh tomato",
            "tomatos": "fresh tomato",
            "cheese": "extra cheese"
        }
        topping = singularize(topping)
        return mapping.get(topping, topping)
    ...
    toppings_input = [normalize_topping_name(t.strip()) for t in (toppings_raw.split(" and ") if " and " in toppings_raw else [toppings_raw])]
